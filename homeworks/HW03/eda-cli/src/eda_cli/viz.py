...
from .core import summarize_dataset
...
def plot_histograms_per_column(
    ...
    bad_out_dir = out_dir / "bad_hist_column"
    numeric_df = df.select_dtypes(include="number")
    dataSummary = summarize_dataset(df)
    ...
    for i, name in enumerate(numeric_df.columns[:max_columns]):
        ...
        col_name = [col.name for col in dataSummary.columns if (col.name == name) and (col.missing_share < min_missing_share)]
        if len(col_name) == 1:
            out_path = out_dir / f"hist_{i+1}_{name}.png"
        else:
            bad_out_dir.mkdir(parents=True, exist_ok=True)
            out_path = bad_out_dir / f"hist_{i+1}_{name}.png"
