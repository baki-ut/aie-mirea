...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    col_uniq = [col.name for col in summary.columns if col.unique > 1]
    flags["has_constant_columns"] = len(col_uniq) == 0

    dif_user_id = [col.name for col in summary.columns if (col.non_null - col.unique == 0) and (col.name == 'user_id')]
    flags["has_suspicious_id_duplicates"] = True if len(dif_user_id) == 1 else False
